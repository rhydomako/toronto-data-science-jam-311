...
all: $(APP_DIR)/fsas.json $(APP_DIR)/all.csv $(APP_DIR)/request_types.csv $(FSA_FILES)
...
$(DATA_DIR)/fsas.csv: $(DATA_DIR)/sr.db
	python process_fsas.py $< $@
...
$(APP_DIR)/all.csv: $(DATA_DIR)/sr.db
	...
	sqlite3 $< -csv "select strftime('%Y-%m',date) || '-01' as _date, count(date),0,0 from sr where location is not null group by _date" >> $@
